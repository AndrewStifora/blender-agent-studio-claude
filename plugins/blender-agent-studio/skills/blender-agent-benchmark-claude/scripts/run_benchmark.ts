...
import { summarizeAgentEvents, type AgentTraceSummary } from "./trace.ts";
import { resolveModelOptions } from "./model-options.ts";
// PORTED: Codex arg/isolation helpers replaced by the Claude CLI adapter.
import { buildClaudeArgs, preflightPinnedMcp, sourceFingerprint } from "./agent-cli.ts";
...
// PORTED: argument construction now lives in agent-cli.ts. Re-exported under
// the original name so run_benchmark.test.ts keeps working.
...
export type ExecutionAggregate = {
  ...
  totalTokens: number;
  totalCostUsd: number;
  costCoverage: { runsWithCost: number; runsTotal: number; complete: boolean };
  costCaveat: string;
};

/**
 * Run-level execution aggregate.
 *
 * ADDED BY THE PORT. The Codex original summed tokens and carried a caveat
 * saying they were "a usage proxy, not a dollar-cost measurement" — true, and
 * it had no cost field to offer instead. The Claude CLI reports
 * `total_cost_usd` on every result event, so cost is now summed directly.
 *
 * Why this matters more than it sounds: in the validated run,
 * cacheWriteInputTokens was 46,879 against inputTokens of 18. `totalTokens`
 * (input + output) came to 352 and missed roughly 99% of what the run
 * consumed. A comparison drawn on totalTokens would be measuring almost
 * nothing, so `totalBilledInputTokens` and `totalCostUsd` are both surfaced.
 *
 * `costCoverage` exists because cost only appears on the result event. A run
 * killed by the harness timeout contributes 0, which silently understates the
 * total — so the count of runs that actually reported a cost travels with it.
 */
export function summarizeExecution(
  items: Array<{ agent: { durationMs: number; trace: AgentTraceSummary } }>,
): ExecutionAggregate {
  const sum = (pick: (trace: AgentTraceSummary) => number) =>
    items.reduce((total, item) => total + pick(item.agent.trace), 0);

  const inputTokens = sum((t) => t.usage.inputTokens);
  const cachedInputTokens = sum((t) => t.usage.cachedInputTokens);
  const cacheWriteInputTokens = sum((t) => t.usage.cacheWriteInputTokens);
  const runsWithCost = items.filter(
    (item) => item.agent.trace.totalCostUsd > 0,
  ).length;

  return {
    totalDurationMs: items.reduce(
      (total, item) => total + item.agent.durationMs,
      0,
    ),
    totalToolCalls: sum((t) => t.toolCalls),
    totalToolFailures: sum((t) => t.toolFailures),
    ...
    totalThinkingTokens: sum((t) => t.usage.reasoningOutputTokens),
    // Every input token the provider billed for, at whatever rate.
    totalBilledInputTokens:
      inputTokens + cachedInputTokens + cacheWriteInputTokens,
    totalTokens: sum((t) => t.usage.totalTokens),
    // Rounded to the cent-thousandth; float sums of many small costs drift.
    totalCostUsd: Number(sum((t) => t.totalCostUsd).toFixed(6)),
    costCoverage: {
      runsWithCost,
      runsTotal: items.length,
      complete: runsWithCost === items.length,
    },
    costCaveat:
      "totalCostUsd is summed from each run's result event and is the field to " +
      "compare conditions on. totalTokens (input + output) excludes cache " +
      "read/write and can understate real spend by orders of magnitude; see " +
      "totalBilledInputTokens. If costCoverage.complete is false, at least one " +
      "run ended without a result event (timeout or crash) and contributed no cost.",
  };
}
...
async function runCodex(options: CodexRunOptions): Promise<{
  ...
  await writeFile(join(options.cwd, "agent-running.json"), JSON.stringify({
    ...
    command: ["claude", ...args, "<prompt-via-stdin>"],
  }, null, 2));
...
async function main(): Promise<void> {
  ...
  const summary = {
    ...
    execution: summarizeExecution(numericResults),
    warning:

...
// PORTED: Codex isolation flags -> Claude CLI adapter; the visual judge now
// goes through judge-cli.ts, which compensates for the missing
// --output-schema / --output-last-message / --image flags.
import { isolatedAgentArgs } from "./agent-cli.ts";
import { runClaudeJudge } from "./judge-cli.ts";
...
type RunSummary = {
  ...
  execution?: {
    ...
    totalErrors: number;
    totalTokens: number;
    // ADDED BY THE PORT. Optional so a summary.json written before the cost
    // aggregation existed still parses.
    totalCostUsd?: number;
...
async function judgePair(options: {
  ...
  // runClaudeJudge writes judge-result.json itself and validates the reply
  // against the schema in TypeScript, because the Claude CLI cannot enforce a
  // JSON schema at the API level. It retries once on a malformed reply.
  const judged = await runClaudeJudge<JudgeResult>(options);
  if (!existsSync(outputPath)) {
    throw new Error(`Visual judge produced no ${outputPath}`);
  }
  const { stdout, stderr, exitCode } = judged;
  return {
      ...
      candidateMinusBaseline:
        baseline.execution && candidate.execution
          ? {
              ...
              // ADDED BY THE PORT. The headline cost delta: what the candidate
              // condition cost over the baseline, in dollars rather than a
              // token proxy that excludes cache read/write.
              costUsd:
                baseline.execution.totalCostUsd === undefined ||
                candidate.execution.totalCostUsd === undefined
                  ? null
                  : Number(
                      (
                        candidate.execution.totalCostUsd -
                        baseline.execution.totalCostUsd
                      ).toFixed(6),
                    ),
              billedInputTokens:
                baseline.execution.totalBilledInputTokens === undefined ||
                candidate.execution.totalBilledInputTokens === undefined
                  ? null
                  : candidate.execution.totalBilledInputTokens -
                    baseline.execution.totalBilledInputTokens,
            }
          : null,
      // A cost delta computed from partial data is worse than none, so say
      // plainly whether both sides reported cost for every run.
      costComparable:
        baseline.execution?.costCoverage?.complete === true &&
        candidate.execution?.costCoverage?.complete === true,
    },
    visualVotes: {
